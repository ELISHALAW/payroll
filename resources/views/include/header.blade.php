        ...
        <div class="flex items-center h-16">
            ...
            <div class="flex-1 hidden md:flex justify-center items-center h-full px-8">
                <div class="flex items-center h-full gap-1">
                    <a href="{{ route('user.profile', $user->id) }}" class="px-2 py-1 text-xs font-black uppercase tracking-widest text-gray-900 bg-gray-100 rounded-full">
                        General
                    </a>

                    @php
                        $navTabs = ['Employment', 'Compensation', 'Leave', 'Family', 'Documents', 'Offday', 'Appraisal'];
                    @endphp

                    @foreach($navTabs as $tab)
                        <a href="#" class="px-2 py-1 text-xs font-bold uppercase tracking-widest text-gray-500 hover:text-gray-900 transition-colors">
                            {{ $tab }}
                        </a>
                    @endforeach
                </div>
            ...
            <div class="flex items-center gap-4">
                ...
                @guest
                    ...
                    <div class="flex items-center gap-2">
                        <a href="{{ route('login') }}" class="px-4 py-2 text-sm font-bold text-gray-600 hover:text-gray-900 transition-colors">
                            Sign In
                        </a>
                        <a href="{{ route('register') }}" class="px-5 py-2 bg-gray-900 text-white rounded-xl text-sm font-bold hover:bg-gray-800 transition-all shadow-sm hover:shadow-md active:scale-95">
                            Register
                        </a>
                    </div>
                @endguest
